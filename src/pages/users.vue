...
export default {
  ...
  methods: {
    ...
    editUser(event) {
      axios.post("http://localhost:4242/update_staff", event.newData)
        .then(response => {
          this.get_user()
    ...
    deleteUser(menu) {
      axios.delete(`http://localhost:4242/delete_staff/${menu.sid}`)
        .then(response => {
          this.$toast.add({ severity: 'success', summary: 'Info', detail: response.data, life: 3000 })
    ...
    get_user() {
      try {
        const response =  axios.get("http://localhost:4242/get_staff/-1")
          .then(response => {
    ...
    create_user() {
      axios.post("http://localhost:4242/create_staff", { "name": this.formState.name, "department": this.formState.department })
        .then(response => {
          this.get_user()
          this.$toast.add({ severity: 'success', summary: 'Info', detail: response.data, life: 3000 })
          this.formState.name = ''
